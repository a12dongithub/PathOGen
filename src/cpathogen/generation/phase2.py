...
class PathOGenDataset(Dataset):
    def __init__(self, image_dir, map_dir, morph_path, image_transforms=None, tokenizer=None):
        self.image_dir = os.fspath(image_dir)
        self.map_dir = os.fspath(map_dir)
        self.morph_path = os.fspath(morph_path)
...
# log_validation removed — not used in concat conditioning mode.
# All validation is handled by inference.run_phase2_validation().
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default=str(ARTIFACT_ROOT / "runs" / "phase2_concat_film"),
    ...
    parser.add_argument(
        "--phase1_unet_checkpoint",
        type=str,
        default=None,
        help=(
            "Path to a specific Phase 1 accelerator checkpoint directory (e.g. artifacts/runs/phase1_domain_adapt/checkpoints/checkpoint-30000). "
            "If provided, UNet weights are overridden from this checkpoint's 'unet/' subfolder."
        ),
    )
    parser.add_argument(
        "--phase2_unet_lr_scale",
        type=float,
        default=0.3,
        help="Learning-rate multiplier for the pretrained UNet during Phase 2.",
    )
    parser.add_argument(
        "--phase2_spatial_lr_scale",
        type=float,
        default=1.0,
    ...
    parser.add_argument("--train-tiles-dir", default=str(TCGA_TILES))
    ...
    parser.add_argument("--validation-morphology-table", default=None)
    parser.add_argument(
    ...
    required_paths = [
        args.train_tiles_dir,
        args.train_spatial_maps_dir,
        args.train_morphology_table,
    ]
    if not all(required_paths):
        raise ValueError("Specify tile, spatial-map, and morphology-table paths")
...
def main(args):
    ...
    if accelerator.is_main_process:
        if args.output_dir is not None:
            ...
            os.makedirs(os.path.join(args.output_dir, "models", "final"), exist_ok=True)
    ...
    # Override UNet weights from a specific Phase 1 checkpoint if requested
    if args.phase1_unet_checkpoint:
        unet_ckpt_path = os.path.join(args.phase1_unet_checkpoint, "unet")
        logger.info(f"Overriding UNet weights from Phase 1 checkpoint: {unet_ckpt_path}")
    ...
    image_column_transform = transforms.Compose(
        [
    ...
    train_dataset = PathOGenDataset(
        image_dir=args.train_tiles_dir,
        map_dir=args.train_spatial_maps_dir,
        morph_path=args.train_morphology_table,
        image_transforms=image_column_transform,
        tokenizer=tokenizer,
    )
    ...
    if args.phase1_unet_checkpoint:
        inference.run_phase1_validation(accelerator, args, global_step=0)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    # Run FID and Visual Validation every 5k steps and on step 1
                if global_step % 5000 == 0 or global_step == 1:
                    ...
                    inference.run_phase2_validation(
    ...
    if accelerator.is_main_process:
        ...
        final_model_dir = os.path.join(args.output_dir, "models", "final")
        final_unet.save_pretrained(os.path.join(final_model_dir, "unet"))
        final_spatial_encoder = unwrap_model(spatial_encoder)
        torch.save(final_spatial_encoder.state_dict(), os.path.join(final_model_dir, "spatial_encoder.pt"))
        torch.save(film_mlps.state_dict(), os.path.join(final_model_dir, "film_mlps.pt"))
        logger.info(f"Saved final UNet, SpatialCondEncoder, and FiLM modules to {final_model_dir}")
