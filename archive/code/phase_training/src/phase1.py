...
def parse_args():
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default=str(ARTIFACT_ROOT / "runs" / "phase1_domain_adapt"),
    ...
    # Sanity checks
    if args.dataset_name is None and args.metadata_file is None:
        raise ValueError("Need either a dataset name or an ImageFolder metadata file.")
...
def main():
    ...
    with accelerator.main_process_first():
        if accelerator.is_main_process:
            logger.info(f"Indexing {args.train_data_dir}... This may take 15-25 minutes for 1M images. Please wait.")
        ...
        if args.dataset_name is not None:
            ...
            if args.metadata_file is not None:
                data_files["train"] = args.metadata_file
            dataset = load_dataset(
                "imagefolder",
                data_files=data_files,
                cache_dir=args.cache_dir,
                download_mode="force_redownload",  # Force regenerate dataset to overwrite the bad cache
            )
            
        if accelerator.is_main_process:
            logger.info("Dataset indexing complete. Initializing training...")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                # Run FID Validation every 10k steps and on step 1
                if global_step % 10000 == 0 or global_step == 1:
                    # Reconstruct the pipeline using the current checkpoint state
                    temp_unet = unwrap_model(unet)
                    if args.use_ema:
                        ema_unet.store(temp_unet.parameters())
                        ema_unet.copy_to(temp_unet.parameters())
                    ...
                    inference.run_phase1_validation(accelerator, pipeline, args, global_step)
                    
                    # Restore parameters so training can keep mutating the real weights
                    if args.use_ema:
                        ema_unet.restore(temp_unet.parameters())
    ...
    if accelerator.is_main_process:
        ...
        pipeline.save_pretrained(final_model_dir)
