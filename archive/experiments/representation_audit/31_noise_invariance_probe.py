...
def generate_batch(unet, vae, spatial_encoder, text_encoder, tokenizer, noise_scheduler, spatial_maps, morph_vectors, grid_seed=42):
    ...
    latents = torch.randn(batch_size, 4, 64, 64, device=DEVICE, dtype=WEIGHT_DTYPE)
...
def main():
    ...
    for i, row in tqdm(tiles_df.iterrows(), total=len(tiles_df)):
        ...
        # Build batch of morphology vectors for noise variance
        batch_morphs = [base_morph] * 11 # 1 baseline + 10 noise variations
        batch_maps = [spatial_data] * 11
    ...
    output_path = Path("artifacts/runs/noise_invariance/metrics/noise_invariance_results.csv")
    ...
    print("\n=============================")
    print("NOISE INVARIANCE RESULTS")
    print(f"Total Tiles Tested: {len(res_df)}")
    print(f"UNI-2h Noise Invariance: {inv_uni:.1f}%")
    print(f"CTransPath Noise Invariance: {inv_ctr:.1f}%")
    print(f"ResNet50 Noise Invariance: {inv_res:.1f}%")
    print("=============================")
