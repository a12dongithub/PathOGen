...
class ResNetSpatialPatchPredictor(nn.Module):
    def __init__(self, morph_dim=16, embed_dim=1536):
        super().__init__()
        self.encoder = timm.create_model('resnet18', pretrained=True, in_chans=3, features_only=True)
...
class UnifiedAblationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Old input: Just the first 3 channels
...
def main():
    ...
    for layer_idx in layers_to_sweep:
        ...
        adapter = ResNetSpatialPatchPredictor(morph_dim=16).to(device)
        ...
        for epoch in range(1, 11):
            train_loss = 0.0
            for img, spatial_3ch, morph, _ in tqdm(train_loader, desc=f"MSE Epoch {epoch}/10"):
                img = img.to(device)
                spatial_3ch = spatial_3ch.to(device)
        ...
        for epoch in range(1, 6):
            for img, spatial_3ch, morph, labels in tqdm(train_loader, desc=f"LoRA Epoch {epoch}/5"):
                spatial_3ch = spatial_3ch.to(device)
                morph = morph.to(device)
        ...
        with torch.no_grad():
            with torch.amp.autocast('cuda'):
                for _, spatial_3ch, morph, _ in test_loader:
                    spatial_3ch = spatial_3ch.to(device)
                    morph = morph.to(device)
                    pred_seq = adapter(spatial_3ch, morph)
        ...
        pd.DataFrame(list(results.items()), columns=['Layer', 'Agreement']).to_csv(
            os.path.join(METRICS_DIR, "high_acc_old_inputs_results.csv"), index=False
        )
