...
class UnifiedAblationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return img_tensor, spatial_3ch, morph, label
...
def collate_fn(batch):
    ...
    return img_tensors, spatial_3chs, morphs, labels
...
def main():
    ...
    # Pre-computing True ViT Pathways on Test Set
    ...
    layer = 12
    print(f"\n{'='*50}\nTraining Adapter + LoRA for Layer {layer} (Robustness Check)\n{'='*50}")
    ...
    for epoch in range(1, epochs + 1):
        adapter.train()
        lora_uni.train()
        for _, spatial_3ch, morph, labels in tqdm(train_loader, desc=f"Epoch {epoch}/{epochs} [Train]"):
    ...
    print("\nTraining Complete! Running Robustness Checks...")
    adapter.eval()
    
    # Modes
    # 1. Real Features
    # 2. Random Spatial (Shuffle batch)
    # 3. Random Morph (Shuffle batch)
    # 4. Fully Random (Shuffle both)
    
    results = {"Real Features": 0, "Random Spatial": 0, "Random Morph": 0, "Fully Random": 0}
    
    with torch.no_grad():
        with torch.amp.autocast('cuda'):
            # REAL FEATURES
            frozen_injected_preds = []
            for _, spatial_3ch, morph, labels in test_loader:
            ...
            results["Real Features"] = np.mean(np.array(frozen_injected_preds) == real_preds) * 100
            
            # RANDOM SPATIAL (Shuffle spatial within batch)
            frozen_injected_preds = []
            for _, spatial_3ch, morph, labels in test_loader:
                idx = torch.randperm(spatial_3ch.nelement())
                spatial_3ch = spatial_3ch.view(-1)[idx].view(spatial_3ch.size())
                ...
                preds = np.argmax(classifier.predict_proba(injected_emb), axis=1)
                frozen_injected_preds.extend(preds)
            results["Random Spatial"] = np.mean(np.array(frozen_injected_preds) == real_preds) * 100
            
            # RANDOM MORPH (Shuffle morph within batch)
            frozen_injected_preds = []
            for _, spatial_3ch, morph, labels in test_loader:
                idx = torch.randperm(morph.size(0))
                morph = morph[idx]
                ...
                preds = np.argmax(classifier.predict_proba(injected_emb), axis=1)
                frozen_injected_preds.extend(preds)
            results["Random Morph"] = np.mean(np.array(frozen_injected_preds) == real_preds) * 100
            
            # FULLY RANDOM
            frozen_injected_preds = []
            for _, spatial_3ch, morph, labels in test_loader:
                s_idx = torch.randperm(spatial_3ch.nelement())
                spatial_3ch = spatial_3ch.view(-1)[s_idx].view(spatial_3ch.size())
                
                m_idx = torch.randperm(morph.size(0))
                morph = morph[m_idx]
                ...
                preds = np.argmax(classifier.predict_proba(injected_emb), axis=1)
                frozen_injected_preds.extend(preds)
            results["Fully Random"] = np.mean(np.array(frozen_injected_preds) == real_preds) * 100

    print("\n" + "="*50)
    print("ROBUSTNESS SPURIOUS CORRELATION CHECK RESULTS")
    print("="*50)
    print(f"Injection Layer: {layer}")
    for k, v in results.items():
        print(f"{k:<18} : {v:.2f}% Agreement with True Frozen ViT")
    print("="*50)
