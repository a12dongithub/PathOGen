...
# CPathoGen portability patch (2026-08-01): the upstream module imports CuPy
# unconditionally even though its CPU postprocessor only uses the NumPy/SciPy
# helpers below. Keep GPU helpers available on CUDA installations while allowing
# the unchanged CPU helpers to import on Apple Silicon and CPU-only systems.
try:
    import cupy as cp
    from cupyx.scipy import ndimage as ndimage_cp
except ImportError:  # pragma: no cover - exercised only without CUDA/CuPy.
    cp = None
    ndimage_cp = None
