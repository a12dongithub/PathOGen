...
This guide supplies the pathology and machine-learning context needed to work on CPathoGen. It is written for engineers and is not a substitute for pathology training. For the exact implemented pipeline and tensor shapes, continue to [Architecture](../architecture/system.md).
...
CPathoGen asks whether a pathology model changes its representation or prediction when selected visual properties are changed while other inputs are held as constant as the generator permits. It therefore has two distinct model families:
...
The Stable Diffusion-derived generator is not a pathology foundation model. UNI2-h and CTransPath are the pathology-pretrained image encoders in the current benchmark; ResNet50 is a conventional comparison.
...
**CellViT++** is described by its authors as a framework that combines nucleus instance segmentation with learned cell embeddings and lightweight classification adaptation; see the [CellViT++ paper](https://arxiv.org/abs/2501.05269). The CPathoGen narrative names CellViT++ as the weak annotator, but its complete source and annotation-generation pipeline are not in this workspace. The bundled NuHTC code is a separate legacy or alternative segmentation strand and should not be silently substituted.
...
### Models and components in this repository
...
| UNI2-h | Pathology foundation encoder | Extracts 1,536-dimensional tile embeddings in current scripts | Gated model access and use terms apply |
| CTransPath | Pathology-pretrained encoder | Alternative tile embedding benchmark | Local architecture/weight compatibility must be preserved |
| ResNet50 | Conventional CNN encoder | Non-foundation baseline | ImageNet features are not pathology-specific by default |
| CellViT++ | Nucleus segmentation/classification framework | Named weak-annotation producer in project narrative | Full pipeline/checkpoint provenance is missing locally |
| NuHTC | Instance-segmentation framework | Bundled legacy/alternative nuclei strand | Not the annotator named by the current CPathoGen narrative |
...
Some posters and older artifacts use the name **ControlNet** for the spatial control. The published ControlNet architecture adds spatial controls to a frozen pretrained diffusion model using connected locked/trainable copies and zero-initialized convolutions; see the [ControlNet paper](https://openaccess.thecvf.com/content/ICCV2023/html/Zhang_Adding_Conditional_Control_to_Text-to-Image_Diffusion_Models_ICCV_2023_paper.html). The current `phase2.py` does **not** implement that architecture. It uses a small spatial encoder followed by direct latent concatenation. Use “concat-conditioned spatial encoder” for the current code unless a true ControlNet implementation is restored.
...
The 16-value vector is standardized over a dataset. A value of `+1` means roughly one training-set standard deviation above the fitted mean, not “100% more” of a biological property. The current repository does not serialize the fitted scaler, which blocks reliable reconstruction of raw units and inference on new data; see [Known issues](../reference/known-issues.md).
...
| PanNuke | Nucleus instances across 19 tissue types with five cell categories | Data for the legacy/alternative segmentation strand | Its taxonomy/annotation process does not automatically match CPathoGen annotations |
| BACH | Breast histology microscopy/WSI data for four-class classification | Downstream feature/classification experiments | Patch labels and contest setup must be preserved when comparing results |

Primary descriptions are available from the [NCI Genomic Data Commons TCGA-BRCA page](https://gdc.cancer.gov/about-data/publications/brca_2012), the [PanNuke paper](https://arxiv.org/abs/2003.10778), and the [BACH challenge paper](https://arxiv.org/abs/1808.04277). A **dataset copy** in this workspace is not necessarily complete or sufficient to reproduce a published cohort; consult [Repository map](../reference/repository-map.md) for what is actually present.
...
## Suggested first-week onboarding path

1. Read [Project overview](project-overview.md) to learn the scientific question and current evidence.
2. Read [Architecture](../architecture/system.md) and trace the five-channel and 16-value contracts into `phase2.py`.
3. Use [Repository map](../reference/repository-map.md) to distinguish canonical code, legacy copies, third-party projects, datasets, and generated artifacts.
4. Read [Setup and workflows](../guides/setup-and-workflows.md), but do not launch a large training run yet.
5. Review every blocker in [Known issues](../reference/known-issues.md), especially data paths, split leakage, missing scaler state, and checkpoint identity.
6. Inspect several real tiles together with their GeoJSON, spatial maps, and computed features. Ask a pathology collaborator to review the visual interpretation rather than relying only on model labels.
7. Reproduce one small feature-extraction or generation smoke test and record exact inputs, checkpoint, environment, seed, and output path.
8. Only then select an experiment from the [paper experiment registry](../research/paper-experiments.md).
...
For implementation claims, current executable code takes priority over posters and old artifacts. For medical terminology, prefer authoritative oncology/pathology references and review project-facing definitions with a qualified pathologist. For model behavior, consult the exact paper, model card, checkpoint license, and code version actually used. Record uncertainty explicitly: this repository is a research prototype and must not be used for diagnosis or treatment decisions.
