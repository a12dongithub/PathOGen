...
# ── Dataset (same as phase2.py) ──
class MiniDataset(torch.utils.data.Dataset):
    def __init__(self, tiles_dir, spatial_maps_dir, stems, transform, tokenizer):
        self.tile_dir = tiles_dir
        self.map_dir = spatial_maps_dir
        self.stems = stems
...
def main():
    ...
    parser.add_argument("--spatial_maps_dir", type=str, required=True)
    parser.add_argument("--num_images", type=int, default=200)
    parser.add_argument("--num_steps", type=int, default=500)
    parser.add_argument("--eval_every", type=int, default=100)
    parser.add_argument("--lr", type=float, default=1e-5)
    parser.add_argument("--output_dir", type=str, default="artifacts/runs/quick_overfit_test")
    ...
    dataset = MiniDataset(args.tiles_dir, args.spatial_maps_dir, selected_stems, transform, tokenizer)
    ...
    for stem in selected_stems[:50]:  # Evaluate on first 50
        img = Image.open(os.path.join(args.tiles_dir, f"{stem}.png")).convert("RGB")
