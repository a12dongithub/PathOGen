...
def main():
    ...
    WEIGHTS_DIR = r"artifacts/models/adapters/deep_mse_weights"
    ...
    # Strictly isolate Layer 12 (Block index 11)
    layer_idx = 11
    
    os.makedirs(WEIGHTS_DIR, exist_ok=True)
    
    print(f"\n{'='*50}\nDeep MSE Training Pipeline for Layer 12\n{'='*50}")
    
    adapter = ResNetSpatialPatchPredictor(morph_dim=20).to(device)
    
    # --- PHASE 1: DEEP MSE Pre-training (30 Epochs) ---
    print("\n--- PHASE 1: DEEP MSE Pre-training (30 Epochs) ---")
    mse_optimizer = optim.AdamW(adapter.parameters(), lr=1e-4, weight_decay=1e-4)
    mse_criterion = nn.MSELoss()
    
    # Learning rate scheduler to help converge smoothly
    scheduler = optim.lr_scheduler.CosineAnnealingLR(mse_optimizer, T_max=30, eta_min=1e-6)
    
    adapter.train()
    for epoch in range(1, 31):
        train_loss = 0.0
        for img, spatial_4ch, morph, _ in tqdm(train_loader, desc=f"MSE Epoch {epoch}/30"):
            img = img.to(device)
        ...
        scheduler.step()
        print(f"MSE Epoch {epoch} Loss: {train_loss / len(train_loader):.6f} | LR: {scheduler.get_last_lr()[0]:.2e}")
        
    os.makedirs(WEIGHTS_DIR, exist_ok=True)
    torch.save(adapter.state_dict(), os.path.join(WEIGHTS_DIR, "adapter_mse_layer_12.pth"))
    ...
    for epoch in range(1, 11):
        for img, spatial_4ch, morph, labels in tqdm(train_loader, desc=f"LoRA Epoch {epoch}/10"):
            spatial_4ch = spatial_4ch.to(device)
            morph = morph.to(device)
    ...
    torch.save(adapter.state_dict(), os.path.join(WEIGHTS_DIR, "adapter_lora_layer_12.pth"))
    ...
    print(f"Layer 12 Top-1 Agreement (Deep MSE): {acc:.2f}%")
    
    with open(os.path.join(METRICS_DIR, "deep_mse_layer12_results.txt"), 'w') as f:
        f.write(f"Layer 12 Top-1 Agreement: {acc:.2f}%\n")
