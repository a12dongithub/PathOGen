...
class UnifiedAblationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Extract Tumor(0), Immune(1), Stroma(2), Epithelial(4)
...
def main():
    ...
    WEIGHTS_DIR = r"artifacts/models/adapters/sweep_weights"
    ...
    layers_to_sweep = [11, 1, 3, 5, 7, 9, 13, 15, 17, 19, 21, 23] # Prioritize Block 11 (Layer 12)
    results = {}
    
    # Save directory for weights
    os.makedirs(WEIGHTS_DIR, exist_ok=True)
    
    for layer_idx in layers_to_sweep:
        print(f"\n{'='*50}\nTraining Adapter + LoRA for Block Index {layer_idx} (Layer {layer_idx+1})\n{'='*50}")
        ...
        for epoch in range(1, epochs + 1):
            adapter.train()
            lora_uni.train()
            for img, spatial_4ch, morph, labels in tqdm(train_loader, desc=f"Epoch {epoch}/{epochs}"):
                spatial_4ch, morph, labels = spatial_4ch.to(device), morph.to(device), labels.to(device)
                optimizer.zero_grad()
                with torch.amp.autocast('cuda'):
        ...
        # Save weights
        torch.save(adapter.state_dict(), os.path.join(WEIGHTS_DIR, f"adapter_layer_{layer_idx+1}.pth"))
        ...
        with torch.no_grad():
            with torch.amp.autocast('cuda'):
                for _, spatial_4ch, morph, _ in test_loader:
                    spatial_4ch, morph = spatial_4ch.to(device), morph.to(device)
                    pred_seq = adapter(spatial_4ch, morph)
        ...
        print(f"Layer {layer_idx+1} Agreement: {acc:.2f}%")
        results[f"Layer {layer_idx+1}"] = acc
        
        # Save running results
        pd.DataFrame(list(results.items()), columns=['Layer', 'Agreement']).to_csv(
            os.path.join(METRICS_DIR, "even_layer_sweep_results.csv"), index=False
        )
