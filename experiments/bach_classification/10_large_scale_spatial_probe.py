...
def add_prediction_label(img, class_name, probability, swap_name):
    ...
    draw.text((10, 32), f"Map: {swap_name[:20]}...", fill="lightgray", font=font_small)
    
    return img

def large_scale_spatial_probe():
    N_TILES = 100
    N_SWAPS = 20
    ...
    # Set seed 123 for different random 100 tiles
    random.seed(123)
    sample_files = random.sample(valid_files, min(N_TILES, len(valid_files)))
    all_stems = list(morph_df.index)
    
    OUTPUT_DIR = Path(r"artifacts/runs/large_scale_spatial_probe")
        ...
        try:
            ...
            for i in range(0, N_SWAPS, BATCH_SIZE):
                ...
                for j, gen_img in enumerate(gen_images):
                    ...
                    # Record
                    result_row = {
                        "Target_Morphology_Stem": stem,
                        "Swap_Spatial_Stem": swap_stem,
            ...
            grid.save(grids_dir / f"{stem}_spatial_swaps_grid.png")
            
        except Exception as e:
            print(f"Error processing {stem}: {e}")
            
    df_results = pd.DataFrame(results)
    output_csv = OUTPUT_DIR / "large_scale_probe_results.csv"
    df_results.to_csv(output_csv, index=False)
    print(f"\nSaved aggregated results to {output_csv}")

if __name__ == "__main__":
    large_scale_spatial_probe()
