...
CLASSES = ["Normal-like", "Basal-like", "HER2-enriched", "Luminal A", "Luminal B"]
...
def overlay_prediction_bar(image, probs):
    ...
    colors = {"Normal-like": "gray", "Basal-like": "red", "HER2-enriched": "magenta", "Luminal A": "blue", "Luminal B": "cyan"}
    color = colors.get(pred_class, "white")
...
def main():
    ...
    # Select 3 random tiles for the test run
    random.seed(42)
    test_files = random.sample(valid_files, 3)
    
    for fpath in tqdm(test_files, desc="Generating Ablation Grids"):
        stem = fpath.stem
        ...
        # Set a fixed seed for the base latent of this tile so we don't get variation
        ...
        out_path = os.path.join(OUTPUT_DIR, f"{stem}_ablation_grid.png")
