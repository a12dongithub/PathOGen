...
def map_to_rgb(spatial_map_np):
    rgb = np.zeros((512, 512, 3), dtype=np.uint8)
    # Background is black. Classes:
    # 0: Neoplastic (Red)
    ...
    for ch in range(5):
        mask = spatial_map_np[:, :, ch] > 0
        for c in range(3):
            rgb[:, :, c][mask] = colors[ch][c]
...
def main():
    ...
    random.seed(777) # Fresh random noises
    target_files = random.sample(valid_files, min(100, len(valid_files)))
    
    # Ensure TCGA-A1-A0SE_x76800_y52224_BL is in the list
    specific_target = spatial_dir / "TCGA-A1-A0SE_x76800_y52224_BL.npz"
    if specific_target.exists() and specific_target not in target_files:
        target_files[0] = specific_target
        
    OUTPUT_DIR = Path(r"artifacts/runs/large_scale_cellular_shifts")
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    
    fractions = [0.0, 0.25, 0.50, 0.75, 1.0]
    
    print(f"Starting Large-Scale Cellular Shift Probe on {len(target_files)} grids...")
    
    for idx, spatial_path in enumerate(tqdm(target_files, desc="Processing grids")):
        stem = spatial_path.stem
        try:
            original_map = np.load(spatial_path)["map"]
            morph_vector = torch.tensor(morph_df.loc[stem].values, dtype=torch.float32)
            ...
            for f in fractions:
                ...
                rgb_maps.append(map_to_rgb(mod_map))
            ...
            # Create 2x5 grid
            grid = Image.new("RGB", (512 * len(fractions), 512 * 2))
            for i in range(len(fractions)):
                # Top row: Generated Images
                grid.paste(labeled_images[i], (512 * i, 0))
                # Bottom row: Spatial Maps
                grid.paste(rgb_maps[i], (512 * i, 512))
                
            out_file = OUTPUT_DIR / f"{stem}_shift_grid.png"
            grid.save(out_file)
            
        except Exception as e:
            print(f"Error processing {stem}: {e}")
            
    print(f"Finished generating grids. Saved to {OUTPUT_DIR}")
